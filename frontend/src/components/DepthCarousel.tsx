...
const DepthCarousel = ({
  ...
  cfgRef.current = {
    count, depth, spread, tilt, tiltDirection, visibleCards,
    falloff, blur, duration, ease, loop, cardWidth, autoplayDelay
  };
  ...
  const layout = useCallback((pos: number) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const blurPx = (cfg.blur as number) > 0
        ? Math.min(cfg.blur as number, (back / Math.max(1, cfg.visibleCards as number)) * (cfg.blur as number))
        : 0;
  ...
  useEffect(() => {
    ...
    const start = () => {
      ...
      autoTimerRef.current = window.setInterval(
        () => { if (!hovered && !focused) navigateByRef.current(1); },
    ...
    >
      <div className="depth-carousel__stage" ref={stageRef}>
        {data.map((item, i) => (
          ...
          >
            <div className="depth-carousel__img" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {item.content}
            </div>
            <span
