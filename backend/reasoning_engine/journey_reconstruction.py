...
class JourneyReconstructor:
    ...
    async def reconstruct(
        ...
        for nid, ndata in node_map.items():
            ...
            is_entry = False
            if n_type in ["ROUTE", "API", "CLI", "EVENT_HANDLER", "CONSUMER", "ENTRY", "PAGE"]:
                is_entry = True
            elif n_type in ["FILE", "UI_COMPONENT", "COMPONENT"] and ("page" in n_path or "screen" in n_path or "app" in n_path):
                is_entry = True
        ...
        try:
            llm_response = await LLMProvider.call_llm(prompt, {"nodes":[], "edges":[]}, schema_instructions=schema_instructions)
