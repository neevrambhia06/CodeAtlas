...
TEST_DATABASE_URL = os.getenv(
    "TEST_DATABASE_URL",
    "postgresql://postgres:postgres@localhost:5432/codeatlas_test"
)
...
try:
    ...
    pytest.skip(f"Could not connect to test database {TEST_DATABASE_URL}. Error: {e}", allow_module_level=True)
...
def test_project_creation_and_retrieval(db_session):
    ...
    saved_repo = db_session.query(Repository).filter(Repository.repo_id == repo_id).first()
...
def test_analysis_job_lifecycle(db_session):
    ...
    # Verify creation
    saved_job = db_session.query(AnalysisJob).filter(AnalysisJob.job_id == job_id).first()
    ...
    updated_job = db_session.query(AnalysisJob).filter(AnalysisJob.job_id == job_id).first()
    ...
    final_job = db_session.query(AnalysisJob).filter(AnalysisJob.job_id == job_id).first()
...
def test_transaction_rollback(db_session):
    ...
    try:
        ...
        with db_session.begin_nested():
            repo_to_update = db_session.query(Repository).filter(Repository.repo_id == repo_id).first()
