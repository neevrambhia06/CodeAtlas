...
class RateLimitingDetector(BaseGapDetector):
    async def detect(
        ...
        routes = [
            n.get("entity", {})
            for n in nodes
            if n.get("entity", {}).get("type") in ["route", "api"]
        ]
        middlewares = [
            n.get("entity", {})
            for n in nodes
            if n.get("entity", {}).get("type") == "middleware"
        ]
        deps = [
            n.get("entity", {}).get("name", "").lower()
            for n in nodes
            if n.get("entity", {}).get("type") == "external_service"
        ]
        ...
        affected = list(
            set([r.get("path", "").split("/")[-1] for r in routes if r.get("path")])
        )[:5]
        if not affected:
...
class ErrorHandlingDetector(BaseGapDetector):
    async def detect(
        ...
        boundaries = [
            n.get("entity", {})
            for n in nodes
            if n.get("entity", {}).get("type")
            in ["route", "controller", "api", "function"]
        ]
        middlewares = [
            n.get("entity", {})
            for n in nodes
        ...
        reasoning = f"CodeAtlas identified {len(boundaries)} asynchronous API and controller boundaries in the repository. "
        if local_handlers:
            ...
            reasoning += (
                "The finding is therefore classified as a missing global error catch."
            )

        affected = list(
            set([r.get("path", "").split("/")[-1] for r in boundaries if r.get("path")])
        )[:5]
        if not affected:
            affected = ["application-core"]
...
class AuthorizationDetector(BaseGapDetector):
    async def detect(
        ...
        protected_routes = [
            n.get("entity", {})
            for n in nodes
            if n.get("entity", {}).get("type") in ["route", "api", "database"]
        ]
...
class GenericGapDetector(BaseGapDetector):
    ...
    async def detect(self, job_id, kg, domains, capabilities, journeys):
        ...
        targets = [
            n.get("entity", {})
            for n in nodes
            if n.get("entity", {}).get("type") in self.affected_types
        ]
        all_deps = [
            n.get("entity", {}).get("name", "").lower()
            for n in nodes
        ...
        affected = list(
            set([r.get("path", "").split("/")[-1] for r in targets if r.get("path")])
        )[:5]
        if not affected:
            affected = ["application-core"]
