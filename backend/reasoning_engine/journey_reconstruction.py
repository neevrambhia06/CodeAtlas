...
class JourneyReconstructor:
    ...
    async def reconstruct(
        ...
        for cap in capabilities:
            cap_name = (
                cap.name if hasattr(cap, "name") and cap.name else cap.get("name")
            )
            if not cap_name:
                continue

            evidence_list = (
                cap.evidence if hasattr(cap, "evidence") else cap.get("evidence", [])
            )
            ...
            for ev in evidence_list:
                ref = (
                    ev.reference
                    if hasattr(ev, "reference")
                    else ev.get("reference", "")
                )
                if not ref:
                    continue
                if (
            ...
            for entry_node in entry_points[:2]:
                ...
                def traverse(node_id: str, depth: int = 0) -> List[str]:
                    ...
                    if n_type == "route" or n_type == "api":
                        ...
                        if "page" in n_path.lower() or "screen" in n_path.lower():
                            ...
                            "context" in n_path.lower()
                            or "service" in n_path.lower()
                            or "lib" in n_path.lower()
                        ):
                            step_type = "FUNCTION"
                        else:
                            step_type = "UI"
                ...
                if len(steps) > 1:
                    has_terminal = any(
                        s.stepType in ["DATABASE", "API", "SERVICE", "EXTERNAL_SERVICE"]
                        for s in steps
                    )
