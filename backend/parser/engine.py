        ...
        for root, dirs, files in os.walk(self.extract_path):
            ...
            for file in files:
                ...
                if file.endswith((".js", ".jsx", ".ts", ".tsx")):
                    self.metadata["languages"].add("JavaScript" if file.endswith(".js") else "TypeScript")
                ...
                elif file.endswith(".sql") or "schema" in file.lower() or "db" in file.lower():
    ...
    def _parse_package_json(self, file_path: str, file_id: str):
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                for fw, key in [("Next.js", "next"), ("React", "react"), ("Express", "express")]:
                    if key in all_deps:
                ...
                for dep in all_deps:
                    if dep in ["stripe", "sendgrid", "twilio", "auth0", "supabase", "firebase"]:
                        ext_id = f"ext:{dep}"
    ...
    def _parse_file(self, full_path: str, rel_path: str, file_id: str) -> dict:
        ...
        try:
            with open(full_path, "r", encoding="utf-8") as f:
                ...
                # 1. Functions
                func_pattern = re.compile(r'(?:async\s+)?(?:function\s+([a-zA-Z0-9_]+)|const\s+([a-zA-Z0-9_]+)\s*=\s*(?:async\s*)?(?:\([^)]*\)|[^=]*)\s*=>|def\s+([a-zA-Z0-9_]+)\s*\()')
                # 2. Classes
                class_pattern = re.compile(r'class\s+([a-zA-Z0-9_]+)')
                # 3. Imports
                import_pattern = re.compile(r'(?:import\s+.*from\s+[\'"]([^\'"]+)[\'"]|require\([\'"]([^\'"]+)[\'"]\)|import\s+[\'"]([^\'"]+)[\'"]|from\s+([^\s]+)\s+import|import\s+([^\s]+))')

                for i, line in enumerate(lines):
                    # Route detection
                    if "app.get(" in line or "router.post(" in line or "app.post(" in line or "router.get(" in line:
                    ...
                    elif ("export default function" in line or "export async function GET" in line or "export async function POST" in line) and ("pages/api" in rel_path or "app/api" in rel_path or "app/" in rel_path):
                        route_id = f"route:{rel_path}:{i}"
                        result["routes"].append(rel_path)
                        result["entities"].append({
                            "id": route_id,
                    ...
                    if func_match:
                        func_name = next(g for g in func_match.groups() if g is not None)
